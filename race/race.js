...
function _datevalues(d3, data) {
  ...
  ) // Ensure d.date is converted to a Date object
    .map(([date, data]) => [date, data]) // Date is already a Date object, no need for conversion
    .sort(([a], [b]) => d3.ascending(a, b)); // Sort by date ascending
}
...
function _color(d3, data) {
  ...
  // 设置颜色比例尺的域为所有不同的 name 值
  scale.domain(names);

  // 返回一个函数，该函数根据 name 获取对应的颜色
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    [
      "category-brands.csv",
      {
        url: new URL("../files/race.csv", import.meta.url),
        mimeType: "text/csv",
        toString,
      },
    ],
